...
class PlayerTimeHistoryFirestoreDataSourceImpl(
    ...
    override fun getPlayerTimeHistory(playerId: Long): Flow<List<PlayerTimeHistory>> = flow {
        ...
        val snapshots = firestore.collection(PLAYER_TIME_HISTORY_COLLECTION)
    ...
    override fun getMatchPlayerTimeHistory(matchId: Long): Flow<List<PlayerTimeHistory>> = flow {
        val currentUserId = firebaseAuth.currentUser?.uid
        if (currentUserId == null) {
            emit(emptyList())
            return@flow
    ...
    override fun getAllPlayerTimeHistory(): Flow<List<PlayerTimeHistory>> = flow {
        val currentUserId = firebaseAuth.currentUser?.uid
        if (currentUserId == null) {
            emit(emptyList())
            return@flow
